...
static uint8_t              *sn_coap_protocol_linked_list_blockwise_payload_search(struct coap_s *handle, const sn_nsdl_addr_s *src_addr_ptr, uint16_t *payload_length);
static bool                  sn_coap_protocol_linked_list_blockwise_payload_compare_block_number(struct coap_s *handle, const sn_nsdl_addr_s *src_addr_ptr, uint32_t block_number);
...
static bool                  sn_coap_handle_last_blockwise(struct coap_s *handle, sn_nsdl_addr_s *src_addr_ptr, sn_coap_hdr_s *received_coap_msg_ptr);
                               ...
                               uint8_t *dst_packet_data_ptr, sn_coap_hdr_s *src_coap_msg_ptr, void *param)
{
    ...
    if (src_coap_msg_ptr->msg_type == COAP_MSG_TYPE_ACKNOWLEDGEMENT &&
        ...
        if (info && info->packet_ptr == NULL) {
            info->packet_ptr = sn_coap_protocol_malloc_copy(handle, dst_packet_data_ptr, byte_count_built);
            if (info->packet_ptr) {
