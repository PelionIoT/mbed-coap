...
TEST(libCoap_builder, build_message_negative_cases)
{
    ...
    CHECK(sn_coap_builder(NULL, NULL, 0) == -2);
    CHECK(sn_coap_builder(NULL, &coap_header, 0) == -2);
    CHECK(sn_coap_builder(buffer, NULL, 0) == -2);

    // Invalid option length
    coap_header.token_ptr = temp;
    CHECK(sn_coap_builder(buffer, &coap_header, 0) == -1);
}

TEST(libCoap_builder, build_message_ok_cases)
{
    CHECK(sn_coap_builder(buffer, &coap_header, 0) == 4);
}
...
TEST(libCoap_builder, build_message_options_token)
{
    ...
    CHECK(sn_coap_builder(buffer, &coap_header, 0) == 6);
}
...
TEST(libCoap_builder, build_message_options_uri_path)
{
    ...
    CHECK(sn_coap_builder(buffer, &coap_header, 0) == 7);
}
...
TEST(libCoap_builder, build_message_options_max_age)
{
    ...
    CHECK(sn_coap_builder(buffer, &coap_header, 0) == 8);
}
...
TEST(libCoap_builder, build_message_options_block1)
{
    ...
    CHECK(sn_coap_builder(buffer, &coap_header, 128) == 8);
}
...
TEST(libCoap_builder, build_message_options_block2)
{
    ...
    CHECK(sn_coap_builder(buffer, &coap_header, 128) == -1);
    ...
    sn_coap_header_check_stub.expectedInt8 = 0;
    CHECK( 4 == sn_coap_builder(buffer, &coap_header, 128) );
}

TEST(libCoap_builder, sn_coap_builder_calc_needed_packet_data_size)
{
    CHECK(sn_coap_builder_calc_needed_packet_data_size(NULL, 0) == 0);
    ...
    header.token_len = 10;
    CHECK(sn_coap_builder_calc_needed_packet_data_size(&header, 0) == 0);
    ...
    CHECK(sn_coap_builder_calc_needed_packet_data_size(&header, 128) == 0);

    header.options_list_ptr->block1_len = 2;
    header.payload_len = 1;
    CHECK(431 == sn_coap_builder_calc_needed_packet_data_size(&header, 128));
    ...
    header.options_list_ptr->block1_len = 2;
    header.payload_len = 1;
    CHECK(722 == sn_coap_builder_calc_needed_packet_data_size(&header, 0));
    ...
    uint16_t val = sn_coap_builder_calc_needed_packet_data_size(&header, 0);
    CHECK( 308 == val );
...
TEST(libCoap_builder, sn_coap_builder_options_build_add_one_option)
{
    ...
    sn_coap_header_check_stub.expectedInt8 = 0;
    CHECK(34 == sn_coap_builder(buffer, &coap_header, 0));

    coap_header.options_list_ptr->proxy_uri_len = 277;
    sn_coap_header_check_stub.expectedInt8 = 0;
    CHECK(285 == sn_coap_builder(buffer, &coap_header, 0));
...
TEST(libCoap_builder, sn_coap_builder_options_build_add_zero_length_option)
{
    ...
    int16_t val = sn_coap_builder(buffer, &coap_header, 0);
    CHECK(val == 9);
