...
TEST_F(libCoap_builder, build_confirmable_response)
{
    ...
    //NULL pointer
    ASSERT_EQ(sn_coap_build_response(NULL, NULL, 0) == NULL);
    ASSERT_EQ(sn_coap_build_response(&handle, NULL, 0) == NULL);
    ASSERT_EQ(sn_coap_build_response(NULL, &coap_header, 0) == NULL);
    ASSERT_EQ(sn_coap_build_response(&handle, &coap_header, COAP_MSG_CODE_RESPONSE_CONTENT) == NULL);
    ...
    ASSERT_EQ(response->msg_type == COAP_MSG_TYPE_ACKNOWLEDGEMENT);
    ASSERT_EQ(response->msg_id == 12);
    ASSERT_EQ(response->msg_code == COAP_MSG_CODE_RESPONSE_CONTENT);
...
TEST_F(libCoap_builder, build_non_confirmable_response)
{
    ...
    retCounter = 1;
    ASSERT_EQ( NULL == sn_coap_build_response(&handle, &coap_header, COAP_MSG_CODE_RESPONSE_CONTENT));

    retCounter = 2;
    response = sn_coap_build_response(&handle, &coap_header, COAP_MSG_CODE_RESPONSE_CONTENT);

    ASSERT_EQ(response != NULL);
    ASSERT_EQ(response->msg_type == COAP_MSG_TYPE_NON_CONFIRMABLE);
    ASSERT_EQ(response->msg_code == COAP_MSG_CODE_RESPONSE_CONTENT);
    ASSERT_EQ(response->token_ptr != NULL);
    ASSERT_EQ(memcmp(response->token_ptr, coap_header.token_ptr, coap_header.token_len) == 0);
    ASSERT_EQ(response->token_len == coap_header.token_len);
...
TEST_F(libCoap_builder, build_message_negative_cases)
{
    ...
    ASSERT_EQ(sn_coap_builder(NULL, NULL) == -2);
    ASSERT_EQ(sn_coap_builder(NULL, &coap_header) == -2);
    ASSERT_EQ(sn_coap_builder(buffer, NULL) == -2);

    // Invalid option length
    coap_header.token_ptr = temp;
    ASSERT_EQ(sn_coap_builder(buffer, &coap_header) == -1);
}

TEST_F(libCoap_builder, build_message_ok_cases)
{
    ASSERT_EQ(sn_coap_builder(buffer, &coap_header) == 11);
}
...
TEST_F(libCoap_builder, build_message_options_token)
{
    ...
    ASSERT_EQ(sn_coap_builder(buffer, &coap_header) == 13);
}
...
TEST_F(libCoap_builder, build_message_options_uri_path)
{
    ...
    ASSERT_EQ(sn_coap_builder(buffer, &coap_header) == 14);
}
...
TEST_F(libCoap_builder, build_message_options_max_age)
{
    ...
    ASSERT_EQ(sn_coap_builder(buffer, &coap_header) == 12);
}
...
TEST_F(libCoap_builder, build_message_options_block2)
{
    ...
    sn_coap_header_check_stub.expectedInt8 = 0;
    ASSERT_EQ( 5 == sn_coap_builder(buffer, &coap_header) );
}

TEST_F(libCoap_builder, sn_coap_builder_calc_needed_packet_data_size)
{
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(NULL) == 0);
    ...
    header.token_len = 10;
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 0);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 304);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 14);

    header.options_list_ptr->max_age = 6;
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 16);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 300);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 377);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 379);
    ...
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 330);

    header.options_list_ptr->observe = 1;
    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 331);
    ...
    ASSERT_EQ(0 == sn_coap_builder_calc_needed_packet_data_size(&header));

    header.options_list_ptr->block1 = 267;
    header.payload_len = 1;
    header.options_list_ptr->block2 = COAP_OPTION_BLOCK_NONE;
    ASSERT_EQ(302 == sn_coap_builder_calc_needed_packet_data_size(&header));

    header.options_list_ptr->block1 = COAP_OPTION_BLOCK_NONE;
    header.options_list_ptr->size1 = 266;
    header.options_list_ptr->use_size1 = true;

    ASSERT_EQ(sn_coap_builder_calc_needed_packet_data_size(&header) == 303);

    header.options_list_ptr->size2 = 266;
    header.options_list_ptr->use_size2 = true;
    header.payload_len = 1;
    ASSERT_EQ(306 == sn_coap_builder_calc_needed_packet_data_size(&header));
    ...
    ASSERT_EQ(14 == sn_coap_builder_calc_needed_packet_data_size(&header));
    free(header.options_list_ptr->uri_query_ptr);

    //Test sn_coap_builder_options_calculate_jump_need "else" case
    header.options_list_ptr = NULL;
    ASSERT_EQ( 12 == sn_coap_builder_calc_needed_packet_data_size(&header) );
...
TEST_F(libCoap_builder, sn_coap_builder_options_build_add_one_option)
{
    ...
    sn_coap_header_check_stub.expectedInt8 = 0;
    ASSERT_EQ(40 == sn_coap_builder(buffer, &coap_header));

    coap_header.options_list_ptr->proxy_uri_len = 277;
    sn_coap_header_check_stub.expectedInt8 = 0;
    ASSERT_EQ(291 == sn_coap_builder(buffer, &coap_header));
