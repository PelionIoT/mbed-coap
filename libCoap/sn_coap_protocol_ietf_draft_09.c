/**************************************************************************//**
 * \file sn_coap_protocol_ietf_draft_09.c
 *
...
static uint16_t              sn_coap_protocol_linked_list_blockwise_msgs_store(sn_nsdl_addr_s *dst_addr_ptr, uint8_t header_len, uint8_t *header_ptr, uint16_t payload_len, uint8_t *payload_ptr, uint8_t one_stored_payload_len);
static coap_blockwise_msg_s *sn_coap_protocol_linked_list_blockwise_msg_search(sn_nsdl_addr_s *src_addr_ptr, uint16_t msg_id);
...
#if SN_COAP_RESENDING_MAX_MSGS_COUNT /* If Message resending is not used at all, this part of code will not be compiled */
 ...
 * \param *ep is the end-point name string (h=)
 * \param *ep_type is the end-point type string (ep=)
 * \param *links is the link format payload string
 *
 * \return Return value 0 given on success. In failure cases:\n
 *          -1 = Failure
 */
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
int8_t sn_coap_register(sn_coap_hdr_s *coap_hdr_ptr, const char *ep, const char *ep_type, const char *links)
{
 ...
 */
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
int8_t 	sn_coap_register_update(sn_coap_hdr_s *coap_hdr_ptr, char *location)
{
	coap_hdr_ptr->msg_code = COAP_MSG_CODE_REQUEST_PUT;
	coap_hdr_ptr->msg_type = COAP_MSG_TYPE_CONFIRMABLE;
	coap_hdr_ptr->msg_id = global_message_id++;
	coap_hdr_ptr->uri_path_len = strlen(location);
	coap_hdr_ptr->uri_path_ptr = (uint8_t*)location;
 ...
 */
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
int8_t 	sn_coap_deregister(sn_coap_hdr_s *coap_hdr_ptr, char *location)
{
...
{
#if SN_COAP_RESENDING_MAX_MSGS_COUNT
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC void sn_coap_protocol_init(void* (*used_malloc_func_ptr)(uint16_t), void (*used_free_func_ptr)(void*))
 *
		...
		uint8_t (*used_tx_callback_ptr)(sn_nsdl_capab_e , uint8_t *, uint16_t, sn_nsdl_addr_s *))
{
    ...
    /* Check if User has given function pointer */
    if (used_malloc_func_ptr != NULL)
    {
        /* Use User's given malloc() function */
        sn_coap_protocol_malloc = used_malloc_func_ptr;
    }
    else
    {
        /* Use library malloc() function */
        sn_coap_protocol_malloc = (void* (*)(uint16_t))&malloc;
    }
    ...
    {
        /* Use User's given free() function */
        sn_coap_protocol_free = used_free_func_ptr;
    }
    else
    {
        /* Use library free() function */
        sn_coap_protocol_free = &free;
    }
    ...
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    /* * * * Create Linked list for storing sent blockwise messages  * * * */
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    /* * * * Create Linked list for storing received blockwise payload * * * */
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC int16_t sn_coap_protocol_build(sn_nsdl_addr_s *dst_addr_ptr, uint8_t *dst_packet_data_ptr, sn_coap_hdr_s *src_coap_msg_ptr)
 *
        ...
        src_coap_msg_ptr->msg_code >= COAP_MSG_CODE_RESPONSE_CREATED)
    {
        ...
        /* (only these messages can be acknowledged because Token option is used as key for stored messages) */
			...
			//{
			{
				...
				message_id = sn_coap_protocol_linked_list_ack_info_search(src_coap_msg_ptr->msg_id,
            ...
            if (message_id >= 0)
            {
                ...
                sn_coap_protocol_linked_list_ack_info_remove(src_coap_msg_ptr->msg_id,
                                                             dst_addr_ptr);
            }
...
//        }
    ...
    else /* Acknowledgement or Reset */
    {
        ...
        /* TODO: Is there possibility that duplication message comes right after that removing */
    ...
    if (src_coap_msg_ptr->payload_len > SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE)
    {
        ...
        /* If not yet other less used options at all */
        ...
        if (fencepost_need != 0)
        {
            /* Because Blockwise option numbers are bigger than 15, Fencepost option is needed */
            /* (Fencepost options has no value, that's why length is zero and one allocated byte is enough) */
            src_coap_msg_ptr->options_list_ptr->fencepost1_len = 0;
            src_coap_msg_ptr->options_list_ptr->fencepost1_ptr = sn_coap_protocol_malloc(1);

            if (src_coap_msg_ptr->options_list_ptr->fencepost1_ptr == NULL)
            {
        ...
        {
            src_coap_msg_ptr->options_list_ptr->fencepost1_ptr = NULL;
        }
        ...
        if (src_coap_msg_ptr->msg_code < COAP_MSG_CODE_RESPONSE_CREATED )
        {
            ...
            {
                sn_coap_protocol_free(src_coap_msg_ptr->options_list_ptr);
                sn_coap_protocol_free(src_coap_msg_ptr->options_list_ptr->fencepost1_ptr);
            ...
            *(src_coap_msg_ptr->options_list_ptr->block1_ptr) = 0x00 +                                     /* First block  (BLOCK NUMBER, 4 MSB bits) */
                                                                ...
                                                                (SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE >> 5); /* Block size   (SIZE, 3 LSB bits) */
        }
        else /* Response message */
        {
        ...
        src_coap_msg_ptr->payload_len = SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE;
    }
    else
    {
        /* Fencepost option is used only with Blockwising, so it is not used now */
    ...
    if (src_coap_msg_ptr->msg_type == COAP_MSG_TYPE_CONFIRMABLE)
    {
        ...
        sn_coap_protocol_linked_list_send_msg_store(dst_addr_ptr,
                                                    byte_count_built,
                                                    dst_packet_data_ptr,
    ...
    /* If blockwising needed */
    if (original_payload_len > SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE)
    {
        /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    	...
    	src_coap_msg_ptr->msg_id = sn_coap_protocol_linked_list_blockwise_msgs_store(dst_addr_ptr,
                                                          byte_count_built - src_coap_msg_ptr->payload_len,
                                                          dst_packet_data_ptr,
                                                          original_payload_len - SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE,
                                                          src_coap_msg_ptr->payload_ptr + SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE,
                                                          SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE);
    }
#endif /* SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE */
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC sn_coap_hdr_s *sn_coap_protocol_parse(sn_nsdl_addr_s *src_addr_ptr, uint16_t packet_data_len, uint8_t *packet_data_ptr)
 *
...
sn_coap_hdr_s *sn_coap_protocol_parse(sn_nsdl_addr_s *src_addr_ptr, uint16_t packet_data_len, uint8_t *packet_data_ptr)
{
    ...
    if (ret_status != 0)
    {
        /* Check if parsed Message type was confirmable, only these messages are confirmed with Reset message */
        if (returned_dst_coap_msg_ptr->msg_type == COAP_MSG_TYPE_CONFIRMABLE)
        {
            /* Set returned status to User */
            returned_dst_coap_msg_ptr->coap_status = COAP_STATUS_PARSER_ERROR_IN_HEADER;

            /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
            /* * * * Store Reset message to Linked list for sending  * * * */
            /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

            ret_status = sn_coap_protocol_store_coap_message_for_sending(src_addr_ptr, COAP_MSG_TYPE_RESET, msg_id);
            ...
            /* Because error in Header, return with coap_status set */
            return returned_dst_coap_msg_ptr;
        }
        else /* Not confirmable message, no need to send Reset message */
        {
    ...
    if (ret_status == -1)
    {
        ...
        if (stored_duplication_msgs_count >= SN_COAP_DUPLICATION_MAX_MSGS_COUNT)
        {
            ...
            sn_coap_protocol_linked_list_duplication_info_remove(stored_duplication_info_ptr->addr_ptr,
                                                                 stored_duplication_info_ptr->port,
    ...
    else /* * * Message duplication detected * * */
    {
        ...
        /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
        /* * * * Store Acknowledgement message to Linked list for sending  * * * */
        /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

        ret_status = sn_coap_protocol_store_coap_message_for_sending(src_addr_ptr, COAP_MSG_TYPE_ACKNOWLEDGEMENT, msg_id);
    ...
    if (returned_dst_coap_msg_ptr->msg_type == COAP_MSG_TYPE_CONFIRMABLE)
    {
        ...
        /* If blockwising used. Note: Blockwise requests are always confirmable */
        if (returned_dst_coap_msg_ptr->options_list_ptr != NULL &&
            (returned_dst_coap_msg_ptr->options_list_ptr->block1_ptr != NULL ||
             returned_dst_coap_msg_ptr->options_list_ptr->block2_ptr != NULL))
        {
            /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
            /* * * * Store blockwise Acknowledgement message to Linked list for sending  * * * */
            /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

            /* TODO: For saving code size, maybe following code should do with sn_coap_protocol_build_msg() function */
            /* so that sn_coap_protocol_build_msg() function would take  options as parameter */
            ...
            int16_t        byte_count_built               = 0;
            uint8_t        more_value                     = 0;

            /* * * First allocate memory for CoAP Acknowledgement message * * */
            ...
            /* Check which one of Block options used */
            if (returned_dst_coap_msg_ptr->options_list_ptr->block1_ptr != NULL)
            {
                src_coap_blockwise_ack_msg_ptr->options_list_ptr->block1_ptr = sn_coap_protocol_malloc(1);

                if (src_coap_blockwise_ack_msg_ptr->options_list_ptr->block1_ptr == NULL)
                {
                    sn_coap_protocol_free(src_coap_blockwise_ack_msg_ptr);
                ...
                more_value = *(returned_dst_coap_msg_ptr->options_list_ptr->block1_ptr);

                /* * * Then build CoAP Acknowledgement message * * */
                src_coap_blockwise_ack_msg_ptr->options_list_ptr->block1_len = 1;
                *(src_coap_blockwise_ack_msg_ptr->options_list_ptr->block1_ptr) = *(returned_dst_coap_msg_ptr->options_list_ptr->block1_ptr) & 0xF0 + /* Block number (BLOCK NUMBER, 4 MSB bits) */
                                                                                  (more_value & 0x08) +                                               /* More flag    (MORE, 1 bit) */
                                                                                  (SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE >> 5);                          /* Block size   (SIZE, 3 LSB bits) */
            }
            else
            {
                src_coap_blockwise_ack_msg_ptr->options_list_ptr->block2_ptr = sn_coap_protocol_malloc(1);
                ...
                more_value = *(returned_dst_coap_msg_ptr->options_list_ptr->block2_ptr);

                /* * * Then build CoAP Acknowledgement message * * */
                src_coap_blockwise_ack_msg_ptr->options_list_ptr->block2_len = 1;
                *(src_coap_blockwise_ack_msg_ptr->options_list_ptr->block2_ptr) = *(returned_dst_coap_msg_ptr->options_list_ptr->block2_ptr) & 0xF0 + /* Block number (BLOCK NUMBER, 4 MSB bits) */
                                                                                  (more_value & 0x08) +                                               /* More flag    (MORE, 1 bit) */
                                                                                  (SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE / 32);                          /* Block size   (SIZE, 3 LSB bits) */
            }
            ...
            /* PUUKKO - this should be done without this */
            //todo: add fencepost option somewhere in building

            src_coap_blockwise_ack_msg_ptr->options_list_ptr->fencepost1_len = 0;
            src_coap_blockwise_ack_msg_ptr->options_list_ptr->fencepost1_ptr = sn_coap_malloc(1);
            memset(src_coap_blockwise_ack_msg_ptr->options_list_ptr->fencepost1_ptr, '1', 1);
            ...
            byte_count_built = sn_coap_builder(dst_ack_packet_data_ptr, src_coap_blockwise_ack_msg_ptr, msg_id);
            ...
            if (byte_count_built < 0)
            {
            	...
            	sn_coap_protocol_free(src_coap_blockwise_ack_msg_ptr->options_list_ptr->block1_ptr);
            	sn_coap_protocol_free(src_coap_blockwise_ack_msg_ptr->options_list_ptr->block2_ptr);
            	//todo: release memory allocations
                return NULL;
            }
            ...
            /* * * Finally send Acknowledgement message and store it to resending Linked list * * */
            /* * * for last message, dont send response, GRS / NSDL / app should do that	  * * */

            if (!((more_value & 0x08) == 0x00))
            	sn_coap_tx_callback(SN_NSDL_PROTOCOL_COAP, dst_ack_packet_data_ptr, byte_count_built, src_addr_ptr);

            sn_coap_protocol_free(dst_ack_packet_data_ptr);


            /* * * * * * * * * * * * * * * * * * * * * * * * */
            /* * * * Handle received blockwise message * * * */
            /* * * * * * * * * * * * * * * * * * * * * * * * */

            /* Check from More bit if this is last block */
            if ((more_value & 0x08) == 0x00)
            {
                ...
                sn_coap_protocol_linked_list_blockwise_payload_store(src_addr_ptr,
                                                                     returned_dst_coap_msg_ptr->payload_len,
                                                                     returned_dst_coap_msg_ptr->payload_ptr);
                ...
                returned_dst_coap_msg_ptr->payload_ptr = sn_coap_protocol_malloc(whole_payload_len);
                returned_dst_coap_msg_ptr->payload_len = whole_payload_len;

                temp_whole_payload_ptr = returned_dst_coap_msg_ptr->payload_ptr;
            ...
            /* Because blockwise message, return with whole Blockwise payload or coap_status set */
            return returned_dst_coap_msg_ptr;
        }
#endif /* SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE */
        ...
        if (returned_dst_coap_msg_ptr->token_ptr != NULL)
        {
            ...
            sn_coap_protocol_linked_list_ack_info_store(msg_id,
                                                        returned_dst_coap_msg_ptr->token_len,
                                                        returned_dst_coap_msg_ptr->token_ptr,
                                                        src_addr_ptr);
        }
        else
        {
    ...
    if (returned_dst_coap_msg_ptr->msg_type == COAP_MSG_TYPE_ACKNOWLEDGEMENT)
    {
             ...
             returned_dst_coap_msg_ptr->options_list_ptr->block2_ptr != NULL))
        {
			...
			if(stored_blockwise_msg_temp_ptr)
			{
				...
				returned_dst_coap_msg_ptr->coap_status = COAP_STATUS_PARSER_BLOCKWISE_ACK;
			}
        }
#endif /* SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE */
    }
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC sn_nsdl_transmit_s *sn_coap_protocol_exec(uint32_t current_time)
 *
 * \brief Sends one CoAP message, if there is any to be sent, by returning it to caller.
 *
 ...
 *
 * \return Return value is pointer to message to be sent. NULL is returned in
 *         following cases:\n
 *          -There is nothing to send\n
 *          -Out of memory (malloc() returns NULL)
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
sn_nsdl_transmit_s *sn_coap_protocol_exec(uint32_t current_time)
{
#if SN_COAP_RESENDING_MAX_MSGS_COUNT /* If Message resending is not used at all, this part of code will not be compiled */
    /* Get node count i.e. count of active resending messages */
    uint8_t stored_resending_msgs_count = sn_linked_list_count_nodes(global_linked_list_resent_msgs_ptr);
#endif

#if SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE /* If Message blockwising is not used at all, this part of code will not be compiled */
    uint8_t stored_blockwise_msgs_count = 0;

    /* * * * Remove old blocwise data * * * */
    sn_coap_protocol_linked_list_blockwise_remove_old_data();

    stored_blockwise_msgs_count = sn_linked_list_count_nodes(global_linked_list_blockwise_sent_msgs_ptr);
#endif
    ...
    /* (these old nodes are removed for case that they are just hanging for some reason and using memory) */
    sn_coap_protocol_linked_list_ack_info_remove_old_ones();

#if SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE /* If Message blockwising is not used at all, this part of code will not be compiled */
    /* Check if there is ongoing blockwise message sendings, Block message sending is not done here after modifications */
//    if (stored_blockwise_msgs_count > 0)
//    {
...
//        {
//            /* If it is time to send this message */
//            if (stored_blockwise_msg_ptr->send_flag == 1)
//            {
//                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
//                /* * * * Allocate and build returned message from stored data  * * * */
//                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
//
//                sn_nsdl_transmit_s *returned_msg_ptr = sn_coap_protocol_build_msg(stored_blockwise_msg_ptr);
//
...
//
//                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * */
//                /* * * *   Manage Blockwise message resending      * * * */
//                /* * * * (all Blockwise messages are confirmable)  * * * */
//                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * */
//
//                /* Store message to Linked list for resending purposes */
//                sn_coap_protocol_linked_list_send_msg_store(stored_blockwise_msg_ptr->send_msg_ptr->dst_addr_ptr,
//                                                            stored_blockwise_msg_ptr->send_msg_ptr->packet_len,
//                                                            stored_blockwise_msg_ptr->send_msg_ptr->packet_ptr,
//                                                            global_system_time + (uint32_t)(RESPONSE_TIMEOUT * RESPONSE_RANDOM_FACTOR));
//
//                /* Return pointer to message to be sent */
...
//    }
    ...
    if (stored_resending_msgs_count > 0)
    {
        ...
        for (i = 0; i < stored_resending_msgs_count; i++)
        {
            ...
            /* If message is sent just once, no resendings needed */
            if (stored_msg_ptr->resending_time == COAP_SEND_MSG_ONCE)
            {
                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
                /* * * * Allocate and build returned message from stored data  * * * */
                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
                ...
                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
                /* * * Remove message from active message sending Linked list  * * */
                /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
            ...
            else /* Normal resending performed */
            {
                ...
                if (current_time >= stored_msg_ptr->resending_time)
                {
                    ...
                    if (stored_msg_ptr->resending_counter >= (typeof(stored_msg_ptr->resending_counter))MAX_RETRANSMIT)
                    {
                        /* All message sending tryings used */
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_send_msg_store(sn_nsdl_addr_s *dst_addr_ptr, uint16_t send_packet_data_len, uint8_t *send_packet_data_ptr, uint32_t sending_time)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static void sn_coap_protocol_linked_list_send_msg_store(sn_nsdl_addr_s *dst_addr_ptr,
                                                        uint16_t send_packet_data_len,
                                                        uint8_t *send_packet_data_ptr,
                                                        uint32_t sending_time)
{
    ...
    if (stored_resending_msgs_count >= SN_COAP_RESENDING_MAX_MSGS_COUNT)
    {
    ...
    for (i = 0; i < stored_resending_msgs_count; i++)
    {
        ...
        if (temp_msg_id == msg_id)
        {
            int8_t mem_cmp_result = memcmp(src_addr_ptr->addr_ptr,
                                           stored_msg_ptr->send_msg_ptr->dst_addr_ptr->addr_ptr,
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_send_msg_remove(sn_nsdl_addr_s *src_addr_ptr, uint16_t msg_id)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static void sn_coap_protocol_linked_list_ack_info_store(uint16_t msg_id,
                                                        uint8_t token_len,
                                                        uint8_t *token_ptr,
                                                        sn_nsdl_addr_s *addr_ptr)
{
    ...
    sn_linked_list_add_node(global_linked_list_ack_info_ptr, stored_ack_info_ptr);
}

/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static int32_t sn_coap_protocol_linked_list_ack_info_search(uint8_t token_len, uint8_t *token_ptr, sn_nsdl_addr_s *addr_ptr)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static int32_t sn_coap_protocol_linked_list_ack_info_search(uint16_t msg_id,
															uint8_t token_len,
                                                            uint8_t *token_ptr,
                                                            sn_nsdl_addr_s *addr_ptr)
{
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_ack_info_remove(uint8_t token_len, uint8_t *token_ptr, sn_nsdl_addr_s *addr_ptr)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static void sn_coap_protocol_linked_list_ack_info_remove(uint16_t msg_id,
                                                         sn_nsdl_addr_s *addr_ptr)
{
    uint16_t         stored_ack_info_count = sn_linked_list_count_nodes(global_linked_list_ack_info_ptr);
    coap_ack_info_s *stored_ack_info_ptr   = sn_linked_list_get_last_node(global_linked_list_ack_info_ptr);
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_ack_info_remove_old_ones(void)
 *
...
static void sn_coap_protocol_linked_list_ack_info_remove_old_ones(void)
{
    ...
    while(removed_ack_info_ptr)
    {
        if ((global_system_time - removed_ack_info_ptr->timestamp)  > SN_COAP_ACK_INFO_MAX_TIME_MSGS_STORED)
        {
            ...
            /* Remove current node moved list automatically to next node
               that is why we can fetch it now by calling get current node. */
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_duplication_info_store(sn_nsdl_addr_s *addr_ptr, uint16_t msg_id)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static int8_t sn_coap_protocol_linked_list_duplication_info_search(sn_nsdl_addr_s *addr_ptr, uint16_t msg_id)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_duplication_info_remove(uint8_t *addr_ptr, uint16_t port, uint16_t msg_id)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static void sn_coap_protocol_linked_list_duplication_info_remove(uint8_t *addr_ptr,
                                                                 uint16_t port,
                                                                 uint16_t msg_id)
{
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_duplication_info_remove_old_ones(void)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_msgs_store(sn_nsdl_addr_s *dst_addr_ptr, uint8_t header_len, uint8_t *header_ptr, uint16_t payload_len, uint8_t *payload_ptr, uint8_t one_stored_payload_len)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static uint16_t sn_coap_protocol_linked_list_blockwise_msgs_store(sn_nsdl_addr_s *dst_addr_ptr,
                                                              uint8_t header_len,
                                                              uint8_t *header_ptr,
                                                              uint16_t payload_len,
                                                              uint8_t *payload_ptr,
                                                              uint8_t one_stored_payload_len)
{
    ...
    while (payload_left > 0)
    {
        ...
        else /* Last block */
        {
            ...
            *block_option_ptr = (i << 4) +                     /* Block number    (BLOCK NUMBER, 4 MSB bits */
            ...
            //todo: add first block messages ID to this, and new ID to first
        ...
        /* Write Header */
        memcpy(stored_blockwise_msg_ptr->send_msg_ptr->packet_ptr,
               header_copy_ptr,
               header_len);
        ...
        /* Write Payload */
        memcpy(stored_blockwise_msg_ptr->send_msg_ptr->packet_ptr + header_len,
               payload_ptr + ((i - 1)  * one_stored_payload_len),
               stored_blockwise_msg_ptr->send_msg_ptr->packet_len - header_len);
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static coap_blockwise_msg_s *sn_coap_protocol_linked_list_blockwise_msg_search(sn_nsdl_addr_s *src_addr_ptr, uint16_t msg_id)
 *
 * \brief Searches stored message from Linked list (Address and Message ID as key)
 *
 * \param *addr_ptr is pointer to Address key to be searched
...
static coap_blockwise_msg_s *sn_coap_protocol_linked_list_blockwise_msg_search(sn_nsdl_addr_s *src_addr_ptr, uint16_t msg_id)
{
    ...
    for (i = 0; i < stored_blockwise_msgs_count; i++)
    {
        /* Get message ID from stored blockwise message */
        uint16_t temp_msg_id = (stored_blockwise_msg_ptr->send_msg_ptr->packet_ptr[2] << 8);
        temp_msg_id += (uint16_t)stored_blockwise_msg_ptr->send_msg_ptr->packet_ptr[3];
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static coap_blockwise_msg_s *sn_coap_protocol_linked_list_blockwise_msg_search_key_addr(sn_nsdl_addr_s *src_addr_ptr)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_msg_remove_current()
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_payload_store(sn_nsdl_addr_s *addr_ptr, uint16_t stored_payload_len, uint8_t *stored_payload_ptr)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static uint8_t *sn_coap_protocol_linked_list_blockwise_payload_search(sn_nsdl_addr_s *src_addr_ptr, uint16_t *payload_length)
 *
...
static uint8_t *sn_coap_protocol_linked_list_blockwise_payload_search(sn_nsdl_addr_s *src_addr_ptr, uint16_t *payload_length)
{
    ...
    {
        int8_t mem_cmp_result = memcmp(src_addr_ptr->addr_ptr,
                                       stored_payload_info_ptr->addr_ptr,
                                       src_addr_ptr->addr_len);
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_payload_remove_oldest()
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_payload_remove_current()
 *
 * \brief Removes current stored blockwise paylod from Linked list
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static uint16_t sn_coap_protocol_linked_list_blockwise_payloads_get_len(sn_nsdl_addr_s *src_addr_ptr)
 *
...
static uint16_t sn_coap_protocol_linked_list_blockwise_payloads_get_len(sn_nsdl_addr_s *src_addr_ptr)
{
    ...
    {
        int8_t mem_cmp_result = memcmp(src_addr_ptr->addr_ptr,
                                       searched_payload_info_ptr->addr_ptr,
                                       src_addr_ptr->addr_len);
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC static void sn_coap_protocol_linked_list_blockwise_remove_old_data(void)
 *
...
/**************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC uint8_t *sn_coap_protocol_blockwise_search_block_option_from_packet_data(uint8_t *searched_packet_data_ptr)
 *
...
uint8_t *sn_coap_protocol_blockwise_search_block_option_from_packet_data(uint8_t *searched_packet_data_ptr)
{
    ...
    {
        uint8_t  option_number     = (*searched_packet_data_ptr >> COAP_OPTIONS_OPTION_NUMBER_SHIFT) + previous_option_number;
        uint16_t option_number_len = (*searched_packet_data_ptr & 0x0F);

        previous_option_number += (*searched_packet_data_ptr >> COAP_OPTIONS_OPTION_NUMBER_SHIFT);
        ...
        {
            option_number_len = 15 + *searched_packet_data_ptr;

            searched_packet_data_ptr++;
        }

        if (option_number == COAP_OPTION_BLOCK1 ||
...
/**************************************************************************//**
 * \fn static void sn_coap_protocol_blockwise_handle_received_ack_block_option(sn_nsdl_addr_s *src_addr_ptr, uint8_t block_option_value)
 *
 * \brief Handles received Block option in acknowledgement message
 *
 * \param *src_addr_ptr is pointer to source address of received Blockwise message
 * \param block_option_value is handled Block option value
 *****************************************************************************/
static void sn_coap_protocol_blockwise_handle_received_ack_block_option(sn_nsdl_addr_s *src_addr_ptr, uint8_t block_option_value)
{
    uint8_t block_number = (block_option_value >> 4);

    /* Check if first acknowledgement, because first acknowledgement */
    /* tells if receiver accepted requested Payload size */
    if (block_number == 0)
    {
        uint8_t accepted_block_size = (block_option_value & 0x07);

        accepted_block_size = (1 << (accepted_block_size + 4));

        /* Check if receiver wants to use smaller Payload size */
        if (accepted_block_size < SN_COAP_BLOCKWISE_MAX_PAYLOAD_SIZE)
        {
            /* Payload sizes must be changed because receiver wants to use smaller Payload size */
            sn_coap_protocol_blockwise_handle_payload_size_change(src_addr_ptr, accepted_block_size);
        }
    }
}
...
sn_nsdl_transmit_s *sn_coap_protocol_build_msg(void *src_msg_ptr)
{
    ...
    memcpy(returned_msg_ptr->packet_ptr,
            ((coap_send_msg_s*)src_msg_ptr)->send_msg_ptr->packet_ptr,
            ((coap_send_msg_s*)src_msg_ptr)->send_msg_ptr->packet_len);
    ...
    memcpy(returned_msg_ptr->dst_addr_ptr->addr_ptr,
            ((coap_send_msg_s*)src_msg_ptr)->send_msg_ptr->dst_addr_ptr->addr_ptr,
            ((coap_send_msg_s*)src_msg_ptr)->send_msg_ptr->dst_addr_ptr->addr_len);
...
/***************************************************************************//**
 * \fn SN_MEM_ATTR_COAP_PROTOCOL_FUNC int8_t sn_coap_protocol_allocate_mem_for_msg(sn_nsdl_addr_s *dst_addr_ptr, uint16_t packet_data_len, void *msg_ptr)
 *
 ...
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
int8_t sn_coap_protocol_allocate_mem_for_msg(sn_nsdl_addr_s *dst_addr_ptr,
 ...
 *
 * \brief Stores CoAP message to Linked list for sending purposes
 *
 * \param *dst_addr_ptr is pointer to destination address where CoAP message will be sent
 * \param msg_type is Message type of sent CoAP message
 * \param msg_id is Message ID of sent CoAP message
 *
 * \return Return value is 0 when succeed and -1 in failure
 *****************************************************************************/
SN_MEM_ATTR_COAP_PROTOCOL_FUNC
static int8_t sn_coap_protocol_store_coap_message_for_sending(sn_nsdl_addr_s *dst_addr_ptr,
                                                              sn_coap_msg_type_e msg_type,
                                                              uint16_t msg_id)
{
    sn_coap_hdr_s *src_coap_msg_ptr           = NULL;
    uint8_t       *dst_packet_data_ptr        = NULL;
    uint16_t       dst_packed_data_needed_mem = 0;
    int16_t        byte_count_built           = 0;

    /* * * First allocate memory for CoAP message * * */

    src_coap_msg_ptr = sn_coap_protocol_malloc(sizeof(sn_coap_hdr_s));

    if (src_coap_msg_ptr == NULL)
    {
        return -1;
    }

    /* * * Then build CoAP message * * */
    src_coap_msg_ptr->msg_type = msg_type;

    /* Then get needed memory count for Packet data */
    dst_packed_data_needed_mem = sn_coap_builder_calc_needed_packet_data_size(src_coap_msg_ptr);

    /* Then allocate memory for Packet data */

    dst_packet_data_ptr = sn_coap_protocol_malloc(dst_packed_data_needed_mem);

    if (dst_packet_data_ptr == NULL)
    {
        sn_coap_protocol_free(src_coap_msg_ptr);

        return -1;
    }

    src_coap_msg_ptr->msg_id = msg_id;

    /* * * Then build message to Packed data * * */
    byte_count_built = sn_coap_builder(dst_packet_data_ptr, src_coap_msg_ptr);

    if (byte_count_built < 0)
    {
        sn_coap_protocol_free(src_coap_msg_ptr);
        sn_coap_protocol_free(dst_packet_data_ptr);

        return -1;
    }

    /* * * Then release memory of CoAP message * * */
    sn_coap_protocol_free(src_coap_msg_ptr);

    /* * * Finally store message to Linked list for waiting sending * * */
    sn_coap_protocol_linked_list_send_msg_store(dst_addr_ptr,
                                                byte_count_built,
                                                dst_packet_data_ptr,
                                                COAP_SEND_MSG_ONCE);
